...
export default function CoreItems() {
    const coreItemsContainer    = useRef<HTMLDivElement | null>(null);
    const { isDesktop }         = useWindowSize();

    useEffect(() => {
        if (isDesktop && coreItemsContainer.current) {
            ...
            gsap.to('.core-item', {
                ...
                stagger: 0.5,
                scrollTrigger: {
                    trigger: coreItemsContainer.current,
                    start: 'top top',
                    end: 'bottom top',
                    scrub: true,
                    pin: true
    ...
    return (
        <section className={styles['core-items']}>
            <div ref={coreItemsContainer} className={styles['core-items__container']}>
                <ul>
